...
function 𝚍species╱𝚍t!(species, t, Par::Parameters)
    ...
    if event == "birth"
        ...
        P𝛅 = [Par.δ[1] * g.total_synonymous_mutations, 
                ...
                Par.δ[4] * g.total_hidden_mutations]
        ...
        if rand() < ∑P𝛅
            ...
            elseif rn < (P𝛅[1] + P𝛅[2] + P𝛅[3]) / ∑P𝛅
                ...
                g′.hidden_mutations += 1
                g′.total_hidden_mutations -= 1
            end
        end
        if mut_flag 
            # add g′ to species
...
# prog = ProgressBar(total = round(Int,1T))
# prog_count = 0
while t < T
    global t
    # @show t
    ...
    if round(1*(t+ δt)) > round(1*t)
        # while prog_count < round(1*(t+ δt))
        #     global prog_count += 1
...
savefig(P, "figures/extended_simulated_$(label)_$(rand_label).png")
savefig(P, "figures/extended_simulated_$(label)_$(rand_label).pdf")